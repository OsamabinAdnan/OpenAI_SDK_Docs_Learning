...
def get_user_profile(id: str) -> str:
    ...
    if id == "1":
        return f"User profile for {id} successfully retrieved."
    else:
...
async def main():
    result = await Runner.run(agent, "get user profile respect to id=1 and show all details of user in result", run_config=run_config)
